...
class BNRCProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(BNRCProcessingBlock, self).__init__()
    ...
    def forward(self, x):
        ...
        # First weight layer with batch normalization and activation
        out = self.layer_dict['conv_0'].forward(x)
        out = F.leaky_relu(self.layer_dict['bn_0'].forward(out))

        # Second weight layer with batch normalization
        out = self.layer_dict['conv_1'].forward(out)
        out = self.layer_dict['bn_1'].forward(out)

        # Residual connection before the final activation
        out += residual
...
class BNProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(BNProcessingBlock, self).__init__()
    ...
    def forward(self, x):
        ...
        # Second weight layer with batch normalization  and activation
        out = self.layer_dict['conv_1'].forward(out)
        out = F.leaky_relu(self.layer_dict['bn_1'].forward(out))
...
class BNDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(BNDimensionalityReductionBlock, self).__init__()
